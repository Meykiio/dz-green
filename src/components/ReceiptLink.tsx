...
export function ReceiptLink({ token }: { token: string }) {
  ...
  return (
    <div className="rounded-lg border border-border bg-card p-4 text-start">
      <p className="text-sm font-medium">{t.receipt.linkTitle}</p>
      <p className="mt-1 text-sm text-muted-foreground">{t.receipt.linkBody}</p>
      <div className="mt-3 flex items-center gap-2">
        ...
        <button
          ...
          aria-label={t.receipt.copyAria}
        >
          <Copy className="size-4" />
        </button>
      </div>
      {copied && <p className="mt-1.5 text-xs text-plant">{t.receipt.copied}</p>}
    </div>
  );
}
