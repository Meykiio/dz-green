...
export function ContactReveal({ kind, id }: { kind: "site" | "fire"; id: string }) {
  ...
  const reveal = async () => {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : "Could not load the contact.");
    } finally {
      setLoading(false);
    }
  };
  ...
  if (!contact.name && !contact.phone) {
    return <p className="text-xs text-muted-foreground">No contact info left by the submitter.</p>;
  }
