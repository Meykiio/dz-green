...
- **Tile-free home map — tried, then reverted on the owner's verdict.** Built `SchematicMap.tsx` (themed canvas + our own outline/borders/names/dots, zero vector-tile requests, verified headlessly) as the owner requested a side-by-side comparison. **The owner preferred the real basemap look** ("I like the live map better") — so the home map is back to `HeroMap` (real basemap) with the 69-wilaya boundaries, and `SchematicMap.tsx` is removed. `mapcn` was researched afterwards as a polish candidate and rejected as the cause/solution — it's the same MapLibre engine with nicer chrome (report in §8 / MAP_ARCHITECTURE_REPORT).
