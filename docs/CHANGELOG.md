...
## 2026-09-02 (eighty-ninth pass) — PlantNet identify fixed for French users — LIVE

- **Owner: "species identification not working no matter what tree."** Ground-truth first: the PlantNet API + key + a real tree image (his own, a pine) identify fine — HTTP 200, `Pinus cembra`, in all three languages. The failure was app-side: `suggestSpecies`'s zod validator allowed only `["en","ar"]`, but the API serves French — and the platform has been shipping French since the 86th pass. Every French identify threw a validation error server-side, was eaten by the fail-soft handler, and surfaced as the confusing-but-quiet "couldn't identify" state. The PlantNet path was untouched since the WebP→JPEG fix (`dddff41`, pre-release) — the French pass modified the UI but never this validator, which is exactly how the gap survived.
- Fix: validator locale `z.enum(["en","ar","fr"])`; `identifyPlant` locale type widened; non-200 responses now log the status + body snippet (429 quota vs 401 bad key vs 400 bad image are distinguishable in prod logs now, where the old generic "PlantNet responded N" was not enough to tell quota from key rot).
- Verified: direct API calls with his image (200 on en/ar/fr); tsc clean; 206/206 tests. Not verified: a full in-browser identify click (the probe's file-input upload wouldn't set photo state in headless — harness limitation, stated plainly; the fixed l10n gap was proven by the API accepting all three languages + the validator rejecting fr before).
- If the owner retests in French and it STILL fails, next suspect is the Vercel `PLANTNET_API_KEY` env var (the local key is verified live; prod's is not directly readable from here).

## 2026-09-02 (eighty-eighth pass) — external audit BUG-01 + BUG-04 (map click crash + silent blank) — LIVE

- **BUG-01 (audit High): clicking a dot added after mount crashed the detail panel.** Root cause reproduced live before touching anything: the mount effect froze the `sites`/`careLogs`/`fires` prop arrays into the click wiring while every other `refs` member was already a ref — a dot added by realtime rendered on the map (the data effect updates sources) but was unresolvable at click time: `featureFor`'s `.find(...)!` returned undefined and threw. Fix: the three arrays travel behind refs (`sitesRef`/`careLogsRef`/`firesRef`) exactly like `dataRef`, and `featureFor` returns null on a genuinely missing id — the click handler no-ops quietly. The non-null assertions were **replaced, not kept**: a missing id stays reachable after the ref fix (refetch swap, a fire resolved and filtered out mid-click), so the guard lives at the call site in `map-layers.ts` and `DetailPanel` never receives undefined. The theme-switch re-wire keeps its mechanism (setStyle wipes layers) but no longer has any data-freshness side effect — everything on `refs` is current by construction now.
- **BUG-04 (audit Low-Med): a failed style-JSON fetch left a silently blank map** behind working UI chrome. `map.once("style.load")` had no timeout and the error handler only recognized `GPUInitializationError` — an OpenFreeMap outage was console-only. Fix (additive, mount-scoped as the audit scoped it): a 15s style budget, and any error before style.load (that's the style fetch itself; tiles/glyphs only come after, and MapLibre never retries it) surfaces the existing "map lost its connection" overlay with its reload button. A late style.load clears the overlay and continues — super-slow networks recover instead of being stuck behind the overlay. `webgl2`/`lost` paths, `MapFailureOverlay`, and all dict copy untouched.
- **Verified — both bugs reproduced live before the fix and re-run after:** the probe submits a care log through the real form in a second tab while the map stays mounted in the first, waits for the realtime dot, clicks it through the real event pipeline. Pre-fix: `PAGEERROR: Cannot read properties of undefined (reading 'site_id')`, no panel. Post-fix: panel opens with the site (Ouargla), zero pageerrors. Style-abort page: pre-fix silent blank; post-fix overlay visible. Probe rows (care log + receipt + rate-limit) deleted after each run; test site verified back to 0 care logs. tsc clean, 206/206 tests, build green. All four touched files under the cap: HeroMap 163, useHeroMapMount 217, map-layers 247, map-data 81.
- Flagged, not fixed (out of scope): a React hydration attribute warning appeared on the care page in one of two identical probe runs — the care page imports none of the four files touched, and the other run was clean, so it's a pre-existing boot race worth watching, not this change. The theme-switch `setStyle` path carries the same theoretical style-fetch exposure as mount did (BUG-04 is mount-scoped per the audit).

## 2026-09-02 (eighty-seventh pass) — EOG flare mask + SOS fix + resolved fires + 250-line rule zeroed — LIVE

- **Flare mask rebuilt on EOG data:** `src/data/flare-zones.ts` is now auto-generated from the EOG Global Gas Flare Analysis 2024 (VIIRS Nightfire annual summary, Earth Observation Group, Payne Institute) — 314 Algerian flare points → 185 clusters (≤5km apart) + 5 live-feed supplements, radius `clamp(spread + 2, 6, 12)` km. It replaced the hand-tuned 13-zone list: authoritative, tighter, and it covers the northern refineries (Arzew, Skikda) the hand list missed. **Regenerated twice mid-pass:** the first stamp min'd every zone at 10km, and a single-point zone 9.5km north-west of Algiers city center black-listed the capital — the unit test asserting Algiers is flare-free caught it; the 6km floor fixed it. Live feed after the fix: 866 raw → 104 shown (99 north, 5 south). The 5 southern points are one-day, non-repeating singles far from every known zone — they stay, under the honest "not ground-verified" label (a new feed day sees a different set; what the mask kills is repeats and known-zone hits).
- **SOS popover never overflows (owner's 390px screenshot):** mobile is screen-fixed (`fixed inset-x-3 top-16`), md+ is anchored under the pill (`sm:absolute sm:end-0 sm:top-full sm:w-64`).
- **Resolved fires leave the public map (owner):** the home route's `fireList` filters to `status === "active"` — the map and list show live fires only; moderation triage is untouched and reopening one in triage brings it back.
- **French content for the seeded announcement drafts:** the 4 drafts now carry real `title_fr`/`body_fr` (live data update), so the marquee renders fully in all three locales.
- **250-line rule zeroed — all six exceptions cleared, every split tsc-verified:** `LocationField.tsx` 332→227 (+ `location-gps.ts`, `location-maps-link.tsx`), `admin/AdminAnnouncementsPanel.tsx` 347→193 (+ `announce-form-bits.tsx`), `AppShell.tsx` 290→249 (+ `AppDrawer.tsx`), `routes/index.tsx` 271→168 (+ `home/ActionCard.tsx`), `_authenticated/activity.tsx` 269→102 (+ `activity/ActivitySections.tsx`), `map/HeroMap.tsx` 262→154 (+ `map/useHeroMapMount.ts` — the mount effect moved verbatim; the RTL control position travels as a prop).
- **Locale switcher cycle → real dropdown** (`LocaleDropdown.tsx`): all three languages one tap away, check on the current, outside-click + Escape close. Ships the owner's original ask from the 3rd-locale launch. En route it exposed a pre-existing gap: the `chrome.aria.switchLocale` label existed only in the FR dict — the EN/AR buttons have been announcing the raw key since the FR pass. Both dicts fixed.
- **Verified:** tsc clean, 206/206 tests (the 6 hotspots failures mid-pass were all the 10km radius stamp — fixed in the data, not by bending the tests), build green, live FIRMS re-check with the new zones (866 raw → 104 shown; 5 southern one-day singles remain, none in any zone), browser probe — map boots, dropdown cycles EN→FR→AR with correct `lang`/`dir` + cookie persistence, drawer opens/closes via X and backdrop, SOS popover on 390px fully inside the viewport with 5 tel links.
