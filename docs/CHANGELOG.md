...
- **Dark-mode map showed the light style.** Once the map mounted immediately (above), it exposed a latent `useTheme` bug: the hook synced the stored theme *before* registering its listener, so a consumer mounting after the initial sync (now the map) stayed on the SSR-safe `"light"` default and opened the light OpenFreeMap style under a dark UI. Fixed `useTheme` (register the listener first + catch up to `current`), and `HeroMap` now reads the real theme/direction from the DOM (`.dark` class + `dir`) to open the map in the correct style with no flash, skipping the theme-effect's first run. Verified in a real browser: dark→dark, light→light (LTR + RTL), map geometry not mirrored.
- **Zoom/recenter controls no longer overlap the action card in RTL.** They're placed on the side opposite the card (bottom-right in LTR, bottom-left in RTL). The +/− zoom and recenter controls were already present and functional.
- **Looser camera constraints (user feedback: too restrictive).** Zoom range widened from 4–16 to 2.5–18, and `maxBounds` expanded from a tight box around Algeria to a generous region ([-30,4]→[45,48]) so panning feels free (the recenter control still snaps back). On phones the initial fit adds bottom padding (~300px) so Algeria frames in the band above the action card instead of half-hidden behind it. Verified in real Chromium at phone + desktop widths.
...
- **The interface is now trilingual.** A dependency-free i18n system (`src/i18n/`) built on the existing stack — no new libraries. Locale is resolved server-side from a `ga-locale` cookie, then `Accept-Language`, then the English default, so SSR emits the correct `<html lang dir>` and translated text on the first byte (no flash, no hydration mismatch). Verified with `curl`: `ga-locale=ar` → `<html lang="ar" dir="rtl">`, `fr` → French LTR, no cookie → English; `Accept-Language: ar-DZ` with no cookie also resolves to Arabic.
  - `config.ts`: locales, RTL set, cookie + BCP-47 helpers, isomorphic cookie / Accept-Language parsers.
  - `locale.server.ts` + `detect.ts`: a `createServerFn` reads the request locale inside its handler (never imported into a loader/component — matches the service-role boundary rule).
  - `index.tsx`: a React-context provider (per-request safe, **not** a module singleton — concurrent SSR requests never share a locale), a typed catalogue accessor (`t.nav.map`), and `Intl` number/date formatters. `format()` fills `{placeholder}` runtime values.
  - Messages live in per-locale namespace slices (`messages/{en,ar,fr}/{shell,content,forms,account,staff}.ts`), each under the 250-line rule. English is the type authority — AR/FR are typed against it, so a missing key fails `tsc`.
- **Language switcher** — a globe control in the top bar (matches the SOS/feedback pill pattern), instant, no page reload; the choice persists in the cookie and restores on the next visit.
- **Every user-facing surface translated:** shell/nav/drawer, home map (headline, stats via `Intl`, legend, layer chips, view toggle), detail panel, list view, plant/care/fire forms (the fire Protection Civile disclaimer is translated, never softened), location & photo inputs, receipt link, feedback dialog, emergency contacts, about, receipt page, auth, activity, and the moderation + admin dashboards. No major hardcoded UI strings remain.
- **RTL:** the layout already used CSS logical properties; fixed the remaining physical ones that would break in Arabic — the shared `Dialog` close button (`right-4` → `end-4`) and header alignment (`sm:text-left` → `sm:text-start`), `SiteList` row alignment, and mirrored directional arrows with `rtl:-scale-x-100`. The map canvas itself is deliberately not flipped.
- **Cleanup:** removed the now-unused `formatDate` from `src/lib/data.ts` (superseded by the locale-aware formatter in `useI18n`).
- **Verification:** `tsc` clean; unit 97/97; public surfaces confirmed via SSR in all three languages. Staff dashboards compile and follow the same wiring but are behind auth (`ssr:false`), so they were not runtime-verified without a session. Meta titles/descriptions remain English (SEO copy) — a possible later pass.
