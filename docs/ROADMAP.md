...
9. **Issue #8 — mobile app. VERIFIED: no mobile code exists; genuine large scope.** Offline queue/sync is the hard part (schema, conflict resolution, photo upload queue). Expo + the existing Supabase backend is the right stack and backend code is reusable. Separate product decision — parked; the web app already works on phones.
