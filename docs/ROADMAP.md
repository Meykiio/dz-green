...
8. **Issue #7 — WebGL2-only basemap; raster fallback. TECHNICALLY TRUE, LOW PRIORITY.** We run `maplibre-gl@^6.3.0`, which dropped WebGL1 entirely (MapLibre v6, April 2026); WebGL2 is required. But WebGL2 is ~97%+ of browsers (available since 2017) and MapLibre ships **no built-in raster fallback** — a hybrid would be a custom build (e.g., OpenLayers) for a tiny audience. Recommended minimal fix instead: catch `GPUInitializationError` and show a clear message. Parked unless adoption data says otherwise.
9. **Issue #8 — mobile app. VERIFIED: no mobile code exists; genuine large scope.** Offline queue/sync is the hard part (schema, conflict resolution, photo upload queue). **Research done 2026-08-19 — `docs/MOBILE_RESEARCH.md`:** Expo + existing Supabase backend is the right stack; recommended architecture = separate `dz-green-mobile` repo, a new write route in the web app (not RLS, not an Edge Function), expo-sqlite outbox for the offline queue, SecureStore session, MLRN + OpenFreeMap with a device spike before committing. Lock-in flags: write path and repo structure must be decided before any code. **Parked — no code until the web foundation is finished.**
