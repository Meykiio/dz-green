...
5. **Issue #2 — the list only shows planted trees, not fires. VERIFIED REAL.** `SiteList` only ever receives `sites` + `careLogs` (index.tsx:101); fires exist on the map and in the legend but the List view omits them entirely. Medium-small: extend the list (or add a fires row per report, respecting the existing `kind` model in `MapFeature`).
6. **Issue #3 — Directions button only partially visible. VERIFIED REAL.** `DetailPanel` is fixed `md:w-[360px]` with `overflow-x-hidden` and two `flex-1` buttons ("Log care for this site" + "Directions"); at 320 px content width the labels clip — worse in French, which is what the reporter used. Small CSS fix (widen panel / let labels wrap / shrink type).
