...
A site-wide, admin-controlled marquee for releases and important notes: a solid strip **above the top bar** (the chrome yields to it — header/sidebar/content/map-height all shift), text traveling in the reading direction (EN left→right, AR right→left), seamless loop, pause on hover, off for reduced-motion. Admin gets a 5th tab («الإعلانات»): **bilingual create/edit** (AR + EN fields — the banner shows the visitor's locale automatically), **color control** (curated contrast-safe palette: ink/plant/care/fire/amber), kind (info/good news/warning), publish/unpublish (one live at a time), delete with two-step confirm. Security: writes are `requireAdmin` service-role fns; anon reads are RLS-limited to the active row; the migration also grants `SELECT` to `service_role` (the empty-list bug the owner caught — RLS bypass ≠ table privilege). Verified: live probe — plant-green strip with Arabic text in AR and English in EN, no overlap (strip bottom = navbar top), marquee CSS present; test row cleaned after.
