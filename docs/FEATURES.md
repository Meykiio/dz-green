...
- **Wilaya boundaries** in green (theme-aware colors — brighter on dark) from the converted polygon data (`src/lib/wilaya-geo.ts`); click a wilaya to zoom into it. **All 69 wilayas with accurate boundaries (2026-08-22)** — the 69-wilaya dataset (MIT, chemsallioua/Algeria69WilayaMap) replaced the coarse Natural Earth 10m shapes; every wilaya including the 11 created in November 2025 has its own polygon, and the dropdowns/leaderboard/moderation cover all 69.
