...
- **Feedback dialog (2026-08-18):** the action card's bottom row gained a "Feedback" button — a dialog (message 1–2000 chars, honeypot, page path) that lands in `public.feedback` via the `submitFeedback` server function. Service-role only; no client access.
