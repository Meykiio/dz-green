...
## 11b. Arabic interface (2026-08-28) — implemented; **trilingual since 2026-09-01 (AR/EN/FR)**

Arabic is the default UI language; English and French are one toggle away (top bar cycles عربي → English → Français, persisted in `ga-locale`, SSR renders in the saved locale via the cookie). Everything is translated from the reviewed master table in `docs/I18N_AR_MASTER.md` — warm-plain MSA, official terms verified against Algerian authorities («الحماية المدنية» 14/1021, Law 18-07 official title), wilaya names in Arabic, full RTL layout with self-hosted Noto Sans/Kufi Arabic, locale-aware dates and numeral agreement. **French (2026-09-01):** full `dict/fr/*` (typed parity enforced by tsc), CLDR plurals (0-1 singular, 2+ plural, verified via `Intl.PluralRules`), `fr-FR` dates, LTR. Announcements are trilingual too (title/body × ar/en/fr — the banner picks the visitor's locale). Tooltips added for the less obvious bits (legend, leaderboard, needs-water, receipt link). See `CHANGELOG.md` forty-ninth pass for the phased list.
...
A fourth map layer next to Trees/Care/Fires: **amber hollow-ring dots** for NASA FIRMS satellite fire detections (VIIRS NOAA-21 NRT, **4-day window** — Suomi NPP retires 2026-11-01, so SNPP was never an option). Fetch is server-side (`/api/public/hotspots`, key stays secret, edge-cached 10 min, 502-with-no-store on failure so clients keep last good data). Filtering: drop `low` confidence (NASA: mostly sun-glint), mask **static flare zones — auto-generated from the EOG Global Gas Flare Analysis 2024** (Earth Observation Group, Payne Institute: 185 clustered zones, radius `clamp(spread + 2, 6, 12)` km — tighter than the hand list, and it also covers the northern refineries; the 6km floor keeps single-point zones from swallowing cities like Algiers) **+ 5 live-feed supplements** (`src/data/flare-zones.ts`), and a **persistence mask**: south of 33.5°N, a pixel repeating on 2+ distinct days of the 4-day window is static infrastructure (northern multi-day fire fronts are never touched). Click opens a detail sheet (confidence, FRP MW, pixel temp °C, acquisition time, satellite + day/night, wilaya when derivable) with honest "not ground-verified" copy, the Protection Civile line, and NASA FIRMS attribution. No schema, no realtime, not in list/board views, no CSP change. Verified: 13 unit tests (parser/filters/masks/country clip), live-feed calibration (results clipped to the wilaya polygons — the API bbox otherwise leaks Morocco/Tunisia/sea), WebKit + Chromium probes (dots render, panel opens, 4 chips no overflow at 320px).
...
A site-wide, admin-controlled marquee for releases and important notes: a solid strip **above the top bar** (the chrome yields to it — header/sidebar/content/map-height all shift), seamless loop, pause on hover, off for reduced-motion. **Multiple announcements can be live at once** — they join into one continuous marquee («إعلان 1 • إعلان 2»; strip color/icon/speed from the newest live row). Travel direction: EN right→left, AR left→right. Admin gets a 5th tab («الإعلانات»): **bilingual create/edit** (AR + EN fields — the banner shows the visitor's locale automatically), **color control** (curated contrast-safe palette: ink/plant/care/fire/amber), **speed control** (10–120 sec/loop), kind (info/good news/warning), publish/unpublish, delete with two-step confirm. Security: writes are `requireAdmin` service-role fns; anon reads are RLS-limited to active rows; `service_role` holds an explicit SELECT grant (the empty-list bug the owner caught — RLS bypass ≠ table privilege). Verified: two drafts live → both joined in EN (reverse) and AR (normal), no overlap; drafts back to inactive.
...
## 11q. What to plant where (2026-09-01) — built, not pushed

The plant form answers "ماذا تغرس في {wilaya}؟" once a wilaya is chosen: up to 4 species chips (tap → fills the species field, suggestion-only like PlantNet). Three data layers: **evidence** — GBIF occurrences harvested per wilaya through exact WKT polygons (`taxonKey=6` after the `class` param turned out to be silently ignored — the first harvest came back full of gulls; WKT length limits needed stride-decimation, one wilaya fell back to bbox), top-10 plants per wilaya in generated `data/wilaya-species.ts`; **climate** — a hand-curated class per wilaya (Mediterranean / semi-arid / arid, standard Algerian climate geography); **the curated matrix** — 19 defensible species with AR/EN names, climate fits, one-line notes, and honest cautions (eucalyptus water-hunger, date palm oases-only, pistachio irrigation). Ranking: guide species with GBIF evidence *inside the wilaya* first, then climate fits. Verified: 7 suggester tests (201 total), tsc, build; probe — Béjaïa shows مصطكى + زيتون (evidence) before Aleppo/Maritime pine (fit).
